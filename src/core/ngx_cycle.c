...
// 开启测试配置文件,如配置文件的语法是否正确,配置文件是否可正确打开 nginx -t
...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
	...
	// conf_ctx http://blog.chinaunix.net/uid-27767798-id-3840094.html
	...
	//  ./nginx -s reopen 
	//  reopen "信号赋值给全局变量ngx_signal，
	//  并将ngx_process赋值为NGX_PROCESS_SIGNALLER=2
	//   开启nginx，再另外开启nginx发送信号
    ...
    /* open the new files */
	// 读conf 文件的时候只是把路径 放到open_files 里面，这里真正的打开
	...
	// ngx_shared_memory_add 添加，这里具体分配
