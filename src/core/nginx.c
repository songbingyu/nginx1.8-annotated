...
main(int argc, char *const *argv)
{
	...
	// 处理argc argv 的一些选项， 如果没有指定，使用默认
	// 主要是获取conf 路径
    if (ngx_process_options(&init_cycle) != NGX_OK) {
        return 1;
    }
	// cacheline setproctitle 
...
// 下面这段代码其实就是获取nginx conf 路径
         ...
         p--)
    {
        if (ngx_path_separator(*p)) {
			// 获取conf 前缀eg： cycle->conf_file = /home/test/nginx/conf/nginx.conf
			// 则 cycle->conf_prefix = /home/test/nginx/conf/
