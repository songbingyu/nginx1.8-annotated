...
typedef enum {
	...
	// 	location级别的uri重写阶段，该阶段执行location基本的重写指令，
	// 也可能会被执行多次；
...
struct ngx_http_phase_handler_s {
	...
	/*
	* 指向下一个phase的第一个handler
	*在ngx_http_core_main_conf_t->phase_engine.handlers数组中的下标
	*/
    ngx_uint_t                 next;
};
...
// 全局配置项
// http://blog.csdn.net/fjslovejhl/article/details/8246330
typedef struct {
	// 存储所有的ngx_http_core_srv_conf_t，元素的个数等于server块的个数。 
	// 因为同一个http块命令可以配置多个server 
	...
	* 监听的所有端口，ngx_http_conf_port_t类型，其中包含socket地址信息。
	  保存ngx_http_conf_port_t结构的元素，
	  在ngx_http_optimize_servers函数中，将会用到ports中的元素来创建监听
	*/
    ngx_array_t               *ports;
...
typedef struct {
	...
	// server的名字数组   ngx_http_server_name_t 
    ngx_array_t                 server_names;

    /* server ctx */
	// 指向包含它的ngx_http_conf_ctx_t结构，
	...
	// 如果在当前server块命令中配置了listen指令，
	// 那么它的值将会设置为1，否则的话就代表没有配置listen指令，
	// 那么ngx就必须为当前server设置默认的监听信息，默认为80端口和所有ip地址。
    unsigned                    listen:1;
#if (NGX_PCRE)
    unsigned                    captures:1;
#endif
...
typedef struct {
	...
	// hash、*wc_head、*wc_tail完成了对哈希表的定义。
	// 该哈希表以server_name作为键（key），
	// 以ngx_http_core_srv_conf_t结构为值（value）。
	// 其中，hash哈希表中的server_name不包括通配符；
	// *wc_head指向的哈希表中，server_name包含前缀通配符；
	// *wc_ tail指向的哈希表中，server_name包含后缀通配符。
    ngx_hash_t                 hash;
