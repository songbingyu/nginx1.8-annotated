...
ngx_http_init_phase_handlers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf)
{
    ...
    find_config_index = 0;
	// ngx_http_rewrite在http module的
	// postconfiguration回调函数中添加REWRITE阶段的处理函数
    use_rewrite = cmcf->phases[NGX_HTTP_REWRITE_PHASE].handlers.nelts ? 1 : 0;
	// ngx_http_access在http module的
	// postconfiguration回调函数中添加ACCESS阶段的处理函数
	...
	* NGX_HTTP_POST_ACCESS_PHASE NGX_HTTP_TRY_FILES_PHASE这4个阶段不能注册模块函数
	*/
	// 多数情况下，执行流程是按照执行链顺序的往前执行，
	// 但在某些执行阶段的checker函数中由于执行了某个逻辑可能需要回跳至之前的执行阶段，
	// 也可能需要跳过之后的某些执行阶段，next字段保存的就是跳跃的目的索引。
    for (i = 0; i < NGX_HTTP_LOG_PHASE; i++) {
    ...
    ngx_http_core_loc_conf_t *pclcf)
{
	...
	// 先对双向链中的location进行排序，
	// 顺序为：exact或inclusive(排序，如果exact的location的名字和inclusive的相同，则exact排在前面) –> 
	// regex（未排序）–> named(字典序排序) –> noname（未排序）。
	...
	// 如果存在named首节点，就分配n + 1的指针数组，
	// 并依次将named类型的location的指针（在链节点的exact字段）
	// 存储到指针数组中，数组的最后一个元素置为空指针，
	// server的named_locations字段指向指针数组。将双向链表从named首节点处断开。
    if (named) {
...
#if (NGX_PCRE)
	// 如果存在regex首节点，就分配r+1的指针数组，
	// 并依次将regex类型的location的指针（在链节点的exact字段）
	// 存储到指针数组中，数组的最后一个元素置为空指针，
	// server层的父location的regex_locations字段指向指针数组。
	// 将双向链表从regex首节点处断开。
    if (regex) {
