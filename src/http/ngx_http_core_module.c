    ...
    ngx_http_location_tree_node_t *node)
{
    ...
    for ( ;; ) {
		...
		// 不得0表示uri和node的name不相等，
		// 这时候三叉树就能加速查找的效率，选择node的左节点或者右节点
        if (rc != 0) {
        ...
        if (len > (size_t) node->len) {
			// 如果这个节点是前缀匹配的那种需要递归tree节点，
			// 因为tree节点后面的子节点拥有相同的前缀。
			// 因为前缀已经匹配到了，所以这里先暂且把loc_conf作为target，
			// 但是不保证后面的tree节点的子节点是否有和uri完全匹配或者更多前缀匹配的。
			// 例如如果uri是/abc,当前node节点是/a,虽然匹配到了location /a,先把/a的location配置作为target，
			// 但是有可能在/a的tree节点有/abc的location，所以需要递归tree节点看一下。
            if (node->inclusive) {

                r->loc_conf = node->inclusive->loc_conf;
				// 设置成again表示需要递归嵌套location，为什么要嵌套递归呢，
				// 因为location的嵌套配置虽然官方不推荐，但是配置的话，
				// 父子location需要有相同的前缀。所以需要递归嵌套location
                rv = NGX_AGAIN;
            ...
            /* exact only */
			// 对于精确匹配的location不会放在公共前缀节点的tree节点中，
			// 会单拉出来一个node和前缀节点平行。
			// 也就是说对于精确匹配 ＝/abcd 和前缀匹配的/abc两个location配置，
			// =/abcd不会是/abc节点的tree节点。=/abcd 只能是／abc的right节点
            node = node->right;
        ...
        if (len == (size_t) node->len) {
            ...
            if (node->exact) {
				...
				//如果还是前缀模式的location，那么需要递归嵌套location了，
				// 需要提前设置loc_conf，如果嵌套有匹配的再覆盖
                r->loc_conf = node->inclusive->loc_conf;
                return NGX_AGAIN;
            }
        }
		...
		// 如果前缀相等，uri的长度比node的长度还要小，
		// 比如node的name是/abc ，uri是/ab,这种情况是/abc 
		// 一定是精确匹配，因为如果是前缀匹配那么／abc 肯定会再／ab的tree 指针里面。
        node = node->left;
    }
}
...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    *cf = pcf;
	// 如果server 指令中没有设listen 选项，use 默认
